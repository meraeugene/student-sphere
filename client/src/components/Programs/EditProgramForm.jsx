...
import { updateProgram } from "../../features/programs/programsSlice";
...
const EditProgramForm = ({
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response) {
        dispatch({ type: "programs/updateProgram", payload: response });
        reset();
        onProgramAdded();
        toggleEditProgramState();
      }
      ...
      <div className="fixed-container z-20 w-full  overflow-y-scroll bg-white opacity-100 transition-all duration-300 md:w-[70%] md:border md:border-r-0 md:border-t-0 md:border-l-[#d2d2d7] lg:w-1/2 rounded-md">
        <div className="p-10 add-user__container">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl play-regular uppercase ">Edit Program</h1>
            <button
              onClick={toggleEditProgramState}
